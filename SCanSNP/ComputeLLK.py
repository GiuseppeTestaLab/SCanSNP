...
def ComputeLikelihood(vcf,GenotypesDF,Counts,DiffOnlyIndex):
	...
	totRefAlleles = LLKGenotypesDF[[ sample+"_RefAl" for sample in VariantsFile.ExtractSamples()]].sum(axis =1)
	totAltAlleles = LLKGenotypesDF[[ sample+"_AltAl" for sample in VariantsFile.ExtractSamples()]].sum(axis =1)
	ScorePerBArcode = pd.DataFrame( columns = list(VariantsFile.ExtractSamples()), index = Counts.barcodes)
	for geno in list(VariantsFile.ExtractSamples()):
...
def ComputeSecondIDLikelihood(SingularLoci_Alt, SingularLoci_Ref, countDf, barcodeList):
	AltSingularGenotype = GenotypesDF.loc[SingularLoci_Alt,[ID+'_AltAl' for ID in list(VariantsFile.ExtractSamples())]].replace(2,1)
	RefSingularGenotype =  GenotypesDF.loc[SingularLoci_Ref,[ID+'_RefAl' for ID in list(VariantsFile.ExtractSamples())]].replace(2,1)
	...
	ScorePerBArcodeAlt = pd.DataFrame( columns = list(VariantsFile.ExtractSamples()), index = barcodeList).fillna(0.0)
	ScorePerBArcodeRef = pd.DataFrame( columns = list(VariantsFile.ExtractSamples()), index = barcodeList).fillna(0.0)
