...
def DFMaker(readList, barcodeList):
	Reads=pd.DataFrame(readList, columns = ["Pos","Barcode","Base","Ref","Alt"])
	print("Storing RefReads...")
	RefReads=Reads[Reads["Base"] == Reads["Ref"]][["Pos","Barcode"]]
	...
	RefReads=RefReads.pivot_table(index = "Pos", columns='Barcode', values = "Counts")
	RefReads.columns =  [Cell + "_RefReads" for Cell in RefReads.columns]
	print("RefReads stored")
	print("Storing AltReads...")
	AltReads=Reads[Reads["Base"] == Reads["Alt"]][["Pos","Barcode"]]
	...
	AltReads=AltReads.pivot_table(index = "Pos", columns='Barcode', values = "Counts")
	AltReads.columns =  [Cell + "_AltReads" for Cell in AltReads.columns]
	print("RefReads and AltReads stored")
	print("Creation of counts matrix...")
	Reads=pd.concat([RefReads,AltReads], axis =1)
	del AltReads
	del RefReads
	for Col in [MissingBarcode for MissingBarcode in list(itertools.chain(*[ (cell+"_RefReads", cell + "_AltReads") for cell  in barcodeList])) if MissingBarcode not in Reads.columns]:
		Reads[Col] = np.nan
	print("Filling NAs...")
	Reads.fillna(0, inplace=True)
	print("Returning filled matrix...")
	return Reads
